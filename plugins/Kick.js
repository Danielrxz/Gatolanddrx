...
const handler = async (msg, { conn }) => {
  ...
  if (targetDigits.size === 0) {
    await conn.sendMessage(chatId, {
      text: "🧐 *Debes mencionar o responder al mensaje del usurio que quieras eliminar.*\n\nEjemplo: *.kick @usuario* o responde al mensaje con *.kick*"
    }, { quoted: msg });
    return;
  }
  ...
  for (const d of targetDigits) {
    ...
    if (d === senderNum) {
      resultados.push(`⚠️ No puedes expulsarte a ti mismo XD (@${d}).`);
      continue;
    }
    if (d === botNum) {
      resultados.push(`⚠️ No puedo expulsarme a mí ⚔️ (@${d}).`);
      continue;
    }
