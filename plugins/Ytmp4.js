...
const handler = async (msg, { conn, text, usedPrefix, command }) => {
  ...
  try {
    ...
    const caption = `🐱 *aquí tienes tu video de YouTube*\n\n` +
      `⚔️ *Título:* ${videoData.title}\n` +
      `⚔️ *Duración:* ${videoData.duration}\n` +
      `⚔️ *Vistas:* ${videoData.views}\n` +
      `⚔️ *Canal:* ${videoData.channel}\n` +
      `⚔️ *Publicado:* ${videoData.publish}\n` +
      `⚔️ *Tamaño:* ${videoData.size}\n` +
      `⚔️ *Calidad:* ${videoData.quality}\n` +
      `⚔️ *Link:* https://youtu.be/${videoData.id}\n\n` +
      `⚔️ *¿No se reproduce?* Usa _${pref}ff_\n\n𖠁 *Prcesando sistema Gatoland...*`;
